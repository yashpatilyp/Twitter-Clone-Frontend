  ...
  const register = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        setUsername('');
        setPassword('');
        setLoading(false); // Set loading state to false when registration process ends
        navigate("/login");
      }
    } catch (error) {
      toast.error(error.response.data.message);
      setLoading(false); // Set loading state to false when registration process ends with error
      console.log(error);
    }
  };
